...
from rabbitears.timings import MIN_1, DAYS
from rabbitears.station_player import (
    StationPlayer,
    PlayerState,
    PlayerOutcome,
    update_status_socket,
)
from rabbitears.reception import (
    ReceptionStatus,
    long_change_effect,
    short_change_effect,
    none_change_effect,
)
from rabbitears.live_schedule_agent import LiveScheduleAgent
...
try:
    from rabbitears.overlay.ticker import run_ticker
...
def start_api_server_with_shutdown_queue(shutdown_queue, command_q):
    from rabbitears.rabbitears_server import rabbitears_server

    rabbitears_server.run_with_shutdown_queue(shutdown_queue, command_q)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RabbitEars TV Player")
    parser.add_argument(
        "-t",
        "--transition",
