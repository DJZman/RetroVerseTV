...
from rabbitears.liquid_manager import LiquidManager
from rabbitears.liquid_schedule import LiquidSchedule
from rabbitears.fluid_builder import FluidBuilder
from rabbitears.sequence_api import SequenceAPI
from rabbitears.rabbitears_server.rabbitears_server import mount_rabbitears_api
...
def print_outcome(success_messages, failure_messages, console):
    console.print(
        "[bold blue underline]Finished running RabbitEars TV.[/bold blue underline]"
    )
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="RabbitEars TV Catalog and Liquid-Schedule Generation"
    )
    parser.add_argument(
        "-g",
        "--graphical_interface",
    ...
    parser.add_argument(
        "-s", "--server",
        action="store_true",
        help="Run the RabbitEars TV web API server after other actions.",
    )
...
def main():
    ...
    if args.graphical_interface:
        try:
            from rabbitears.ux.ux import StationApp
    ...
    if args.server or len(sys.argv) <= 1:
        info = "\nRabbitEars web server is running on this machine. You can log into the web gui at http://localhost:4242 to manage catalogs and schedules\n"
        print()
        console.print(Panel.fit(info, title="RabbitEars TV", subtitle="It's Up To You.", border_style=style.Style(color="blue")))
        print()
