...
// Theme management
function initTheme() {
    const savedTheme = localStorage.getItem('rabbitears-theme') || 'default';
    setTheme(savedTheme);
...
function setTheme(themeName) {
    ...
    localStorage.setItem('rabbitears-theme', themeName);
}
...
async function createMainMenu() {
    ...
    let playerMenuItems = '';
    try {
        const response = await window.rabbitearsApi.get('player/status/queue_connected');
    ...
    return `
        <div class="pure-menu pure-menu-horizontal custom-menu" id="mainMenu">
            <a href="#" class="pure-menu-heading">RabbitEars TV</a>
            <span class="menu-toggle" onclick="window.rabbitearsCommon.toggleMenu()">&#9776;</span>
...
function createLogDisplay() {
    return `
        <div id="log-frame" class="log-container" style="display:none;">
            ...
            <pre id="log-content" class="rabbitears-terminal"></pre>
        </div>
    `;
}
...
async function fetchStationSummary() {
    try {
        const resp = await window.rabbitearsApi.get('summary');
...
async function fetchCatalog(networkId) {
    try {
        const resp = await window.rabbitearsApi.get(`catalogs/${networkId}`);
...
async function fetchScheduleSummary(networkId) {
    try {
        const resp = await window.rabbitearsApi.get(`summary/schedules/${networkId}`);
        return resp;
    } catch (e) {
        return null;
    }
...
async function fetchSchedule(networkId, start, end) {
    try {
        ...
        const resp = await window.rabbitearsApi.get(url);
