# 📡 RabbitEars TV Server API Guide

Welcome to the RabbitEars TV Server API! This is your one-stop guide to understanding and using the comprehensive REST API that powers your broadcasting station. Whether you're setting up your system or controlling live playback, this API has got you covered.
...
```bash
python3 rabbitears_station.py --server
```
...
The RabbitEars TV API is organized around two main concepts:
...
We're constantly improving RabbitEars TV! Here's what's on the roadmap:
...
- **Source Code**: Check `rabbitears_server.py` and the `api/` directory for implementation details

Remember: RabbitEars TV is designed to be fun and easy to use. If something seems complicated, we probably need to make it simpler! 🎉
