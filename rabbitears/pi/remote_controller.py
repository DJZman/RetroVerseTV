...
RabbitEars_HOST = os.getenv('RabbitEars_HOST', '127.0.0.1')
RabbitEars_PORT = os.getenv('RabbitEars_PORT', '4242')
RabbitEars_BASE_URL = f"http://{RabbitEars_HOST}:{RabbitEars_PORT}"
...
SYSTEMCTL_TO_TOGGLE = [
    'rabbitears.service',           # Field Player
    'rabbitears-cable-box.service', # Cable Box
    'rabbitears-osd.service',       # On-Screen Display
]
    ...
    with input_lock:
        if channel_input:
            try:
                ...
                response = requests.get(f'{RabbitEars_BASE_URL}/player/channels/{channel_number}')
                if response.ok:
...
def show_guide_pressed():
    ...
    try:
        response = requests.post(f'{RabbitEars_BASE_URL}/player/channels/guide')
        if response.ok:
...
def volume_up_pressed():
    ...
    try:
        response = requests.get(f'{RabbitEars_BASE_URL}/player/volume/up')
        if response.ok:
            data = response.json()
...
def volume_down_pressed():
    ...
    try:
        response = requests.get(f'{RabbitEars_BASE_URL}/player/volume/down')
        if response.ok:
            data = response.json()
...
def mute_pressed():
    ...
    try:
        response = requests.get(f'{RabbitEars_BASE_URL}/player/volume/mute')
        if response.ok:
            data = response.json()
...
def channel_up_pressed():
    ...
    global current_channel, last_channel
    try:
        response = requests.get(f'{RabbitEars_BASE_URL}/player/channels/up')
        if response.ok:
            ...
            try:
                ...
                for attempt in range(max_attempts):
                    time.sleep(0.05)
                    status_response = requests.get(f'{RabbitEars_BASE_URL}/player/status')
...
def channel_down_pressed():
    ...
    global current_channel, last_channel
    try:
        response = requests.get(f'{RabbitEars_BASE_URL}/player/channels/down')
        if response.ok:
...
def last_channel_pressed():
    ...
    if last_channel is not None:
        try:
            ...
            response = requests.get(f'{RabbitEars_BASE_URL}/player/channels/{last_channel}')
            if response.ok:
...
def end_pressed():
    ...
    try:
        response = requests.get(f'{RabbitEars_BASE_URL}/player/commands/stop')
        if response.ok:
...
def toggle_services():
    """Toggle RabbitEars TV services on/off - true power button behavior"""
    if not should_allow_press('power_stop'):
        return  # Debounced - ignore this press

    try:
        # Check if rabbitears.service is active to determine current state
        result = subprocess.run(
            ['systemctl', '--user', 'is-active', 'rabbitears.service'],
            capture_output=True,
            text=True,
            timeout=5
        )
...
def find_input_device(device_spec=None):
    ...
    if default is False and (device_spec or os.getenv('RabbitEars_INPUT_DEVICE')):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='RabbitEars TV Remote Controller',
...
Environment variables:
  RabbitEars_INPUT_DEVICE         Device specification (same format as -d)
  RabbitEars_HOST                 Server host (default: 127.0.0.1)
  RabbitEars_PORT                 Server port (default: 4242)
        """
    )
    parser.add_argument(
        '-d', '--device',
    ...
    device_spec = args.device or os.getenv('RabbitEars_INPUT_DEVICE')

    print(f"Remote Controller started. Connecting to {RabbitEars_BASE_URL}")
...
def mpv_command_pressed(action):
    """Send a non-interrupting mpv runtime command to RabbitEars."""
    ...
    try:
        response = requests.post(
            f'{RabbitEars_BASE_URL}/player/mpv/{endpoint}',
            timeout=2
        )
