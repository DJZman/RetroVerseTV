# 📱 RabbitEars TV Remote Controller

Control your RabbitEars TV server with any IR remote using Flirc USB or customize keyboard mappings for direct control.
...
### Server Settings
Change the RabbitEars TV server location:
```bash
# Environment variables
RabbitEars_HOST=192.168.1.100 RabbitEars_PORT=8080 sudo python3 remote_controller.py

# Or edit the script directly
RabbitEars_HOST = '192.168.1.100'
RabbitEars_PORT = '8080'
```
...
RabbitEars_INPUT_DEVICE=keyboard sudo python3 remote_controller.py

# Combine with other settings
RabbitEars_HOST=192.168.1.100 RabbitEars_INPUT_DEVICE=1 sudo python3 remote_controller.py
```
...
RabbitEars_HOST=192.168.1.50 sudo python3 remote_controller.py
```
...
3. **Check server**: Verify RabbitEars TV is running on correct host/port
