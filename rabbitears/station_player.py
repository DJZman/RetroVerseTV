...
from rabbitears.guide_tk import guide_channel_runner, GuideCommands
...
try:
    from rabbitears.webrender.web_render import web_render_runner
...
from rabbitears.liquid_manager import LiquidManager, PlayPoint, ScheduleNotFound, ScheduleQueryNotInBounds
...
class StationPlayer:
    ...
    def _show_now_playing(self, file_path):
        ...
        # Start new overlay
        try:
            from rabbitears.overlay.now_playing import run_now_playing
        ...
        try:
            if os.path.exists(file_path) or is_stream or AutoBumpAgent.is_autobump_url(file_path):
                ...
                if media_type == 'audio' and content_type == 'feature':
                    ...
                    self._close_now_playing()
                    try:
                        from rabbitears.nfo_agent import NFOAgent
